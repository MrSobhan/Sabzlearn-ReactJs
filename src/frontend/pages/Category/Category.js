...
export default function Category() {
  ...
  useEffect(() => {
    // fetch(`http://localhost:4000/v1/courses/category`)
    //   .then((res) => res.json())
    //   .then((allCourses) => {
    //     console.log(allCourses, categoryName, localStorageData);
    ...
    fetch(`http://localhost:4000/v1/category`)
      .then((res) => res.json())
      .then((allCategory) => {
        const categoryTitle = allCategory.find(
        ...
        if(categoryTitle){
          ...
          .then((res) => res.json())
          .then((allCourses) => {
            const AllCoureseFilter = allCourses.filter(
              (course) => course.categoryID == categoryTitle.title
            );
            setCourses(AllCoureseFilter);
            setOrderedCourses(AllCoureseFilter);
          });
        }else{setShownCourses([])}
      });
  }, [categoryName]);
